...
This will produce a bunch of debug output as aiohttp steps through the request process.  Omitting those lines,
we'll eventually see the length of each html response plus a random nonce:
...
.. code-block:: console
    ...
    ...
    ...
    DEBUG:asyncio:poll 15446.009 ms took 74.108 ms: 1 events
    {
...
run_once.py above,  I've dropped the logging for readability:
...
In the following sample I've added a function to handle SIGINT gracefully, which demonstrates how to shut
down the periodic task and event loop on CTRL + C.
